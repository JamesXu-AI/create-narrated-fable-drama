...
def _validate_project_direction(text: str, context: dict[str, object]) -> None:
    headers, rows = _table_after_heading(text, "## Project Direction")
    if headers != ["Field", "Value"]:
        raise StoryboardValidationError(
            "Project Direction must use Field | Value"
        )
    values = {row[0]: row[1] for row in rows}
    if values.get("Aspect Ratio") != "16:9":
        raise StoryboardValidationError(
            "Project Direction Aspect Ratio must be 16:9"
        )
    if not values.get("Target Country") or not values.get("Target Language"):
        raise StoryboardValidationError(
            "Project Direction must preserve target country and language"
        )
    expected = {
...
def _validate_speech_transition_plan(task_dir: Path, text: str) -> int:
    headers, rows = _table_after_heading(text, "## Speech Transition Plan")
    if tuple(headers) != SPEECH_TRANSITION_HEADERS:
        raise StoryboardValidationError(
            "Speech Transition Plan columns differ from the current contract"
        )
    screenplay_text = (
        task_dir / "screenplay-writer" / "screenplay.md"
    ).read_text(encoding="utf-8")
    screenplay_lines: dict[str, tuple[str, str, str]] = {}
    pattern = re.compile(
        r'(L-[0-9]{3,}); speaker=([a-z0-9]+(?:-[a-z0-9]+)*); '
        r'mode=([a-z_]+); gate=[^;]+; transition=[^;]+; '
        r'delivery=[^;]+; text="([^"]+)"'
    )
    for match in pattern.finditer(screenplay_text):
        screenplay_lines[match.group(1)] = (
            match.group(2),
            match.group(3),
            match.group(4),
        )
    if not screenplay_lines:
        raise StoryboardValidationError(
            "Approved screenplay has no valid spoken-line authority"
        )
    row_lines = [row[2] for row in rows]
    if row_lines != list(screenplay_lines):
        raise StoryboardValidationError(
            "Speech Transition Plan must cover every screenplay Line exactly once "
            "in order"
        )
    for index, row in enumerate(rows, start=1):
        if row[0] != f"T-{index:03d}":
            raise StoryboardValidationError(
                "Speech Transition IDs must be consecutive from T-001"
            )
        line_id = row[2]
        speaker, mode, exact_text = screenplay_lines[line_id]
        if row[3] != f"{speaker}; mode={mode}":
            raise StoryboardValidationError(
                f"{line_id} speaker or delivery mode differs from screenplay"
            )
        if any(
            not cell.strip() or cell.strip().casefold() in {"none", "tbd", "same"}
            for cell in row[4:]
        ):
            raise StoryboardValidationError(
                f"{line_id} lacks a concrete natural speech handoff"
            )
        if len(re.findall(rf"\b{re.escape(line_id)}\b", text)) < 2:
            raise StoryboardValidationError(
                f"{line_id} must appear in Speech Transition Plan and its Ordered Shot"
            )
...
def _validate_storyboard_speech_rate(
    task_dir: Path, text: str
) -> dict[str, object]:
    generation_headers, generation_rows = _table_after_heading(
        text, "## Generation Plan"
    )
    if "Segment" not in generation_headers or "Duration Seconds" not in generation_headers:
        raise StoryboardValidationError(
            "Generation Plan lacks Segment or Duration Seconds"
        )
    segment_index = generation_headers.index("Segment")
    duration_index = generation_headers.index("Duration Seconds")
    try:
        duration_by_segment = {
            row[segment_index]: float(row[duration_index]) for row in generation_rows
        }
    except ValueError as exc:
        raise StoryboardValidationError(
            "Generation Plan contains a nonnumeric duration"
        ) from exc
    ...
    screenplay_lines = {
        ...
        for match in screenplay_pattern.finditer(screenplay_text)
    }
    reports: list[dict[str, object]] = []
    observed_line_ids: list[str] = []
    matches = list(SEGMENT_HEADING_RE.finditer(text))
    for index, heading in enumerate(matches):
        segment_id = f"segment-{int(heading.group(1)):03d}"
        end = matches[index + 1].start() if index + 1 < len(matches) else len(text)
        section = text[heading.end() : end]
        headers, rows = _table_after_heading(section, "### Ordered Shots")
        if tuple(headers) != ORDERED_SHOT_HEADERS:
            raise StoryboardValidationError(
                f"{segment_id} Ordered Shots columns differ from contract"
            )
        prior_end = 0.0
        for shot_index, row in enumerate(rows, start=1):
            cell = row[8]
            if "L-" not in cell:
                continue
            ...
            if match is None:
                raise StoryboardValidationError(
                    f"{segment_id} Shot {shot_index} speech must use the exact "
                    "Line/window/speaker/mode/transition/text prefix"
                )
            line_id, start, finish, speaker, mode, transition, exact_text = match.groups()
            authority = screenplay_lines.get(line_id)
            if authority != {"speaker": speaker, "mode": mode, "text": exact_text}:
                raise StoryboardValidationError(
                    f"{line_id} differs from screenplay speaker, mode, or exact text"
                )
            start_seconds = float(start)
            end_seconds = float(finish)
            duration = duration_by_segment.get(segment_id)
            if (
                duration is None
                or start_seconds < prior_end
                or end_seconds <= start_seconds
                or end_seconds > duration
            ):
                raise StoryboardValidationError(
                    f"{line_id} has an invalid or overlapping Segment-local window"
                )
            try:
                rate = require_speech_rate(
                    line_id=line_id,
                    text=exact_text,
                    window_seconds=end_seconds - start_seconds,
                    stage=f"storyboard {segment_id}/Shot {shot_index}",
                )
            ...
            observed_line_ids.append(line_id)
            prior_end = end_seconds
    if observed_line_ids != list(screenplay_lines):
        raise StoryboardValidationError(
            "Storyboard speech-rate gate must cover every screenplay Line once in order"
        )
    return {
        "status": "PASS",
...
def _validate_character_state_plan(
    ...
    for row in rows:
        ...
        required = comma_values(required_cell)
    ...
    for key, upstream in expected.items():
        ...
        required_screenplay = set(comma_values(upstream[6]))
...
def validate_storyboard(task_dir: Path) -> dict:
    ...
    storyboard_document = StoryboardDocument.load(
        task_dir,
        error_type=StoryboardValidationError,
    )
    storyboard = storyboard_document.path
    ...
    _validate_project_direction(text, project_context)
    matches = list(SEGMENT_HEADING_RE.finditer(text))
    ...
    speech_transition_count = _validate_speech_transition_plan(task_dir, text)
    speech_rate_gate = _validate_storyboard_speech_rate(task_dir, text)
    ...
    return {
        ...
        "speech_transition_count": speech_transition_count,
        "speech_rate_gate": speech_rate_gate,
