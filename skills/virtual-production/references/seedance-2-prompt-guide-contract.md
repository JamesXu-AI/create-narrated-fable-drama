...
- `multimodal_reference`: make a new clip from named identity, location, voice,
  state, or style references.
...
Declare every Storyboard provider token before its first use. Give each medium a
single job: identity, appearance state, location, recent temporal state, voice, or
ambience. Use stable readable subject names; internal asset IDs mean nothing to
the provider.
...
## Ordered audiovisual shots

Each shot states one dominant camera behavior, visible subject action and
...
- lip-sync owner and closed-mouth listeners;
...
- whether the same established character voice crosses the cut;
- how ambience bridges the transition.
