...
from narrated_fable_drama.contracts.boundary import (
    NON_CUT_REVIEW_TRANSITION_CLASSES,
)
...
def _read_json(path: Path, *, label: str) -> dict[str, Any]:
    return load_json_object(
        path,
        label=label,
        error_type=IncrementalBoundaryPrecheckError,
    )
...
def classify_technical_precheck(
    ...
    if transition_class in NON_CUT_REVIEW_TRANSITION_CLASSES:
        return (
    ...
    record = {
        ...
        **contract,
        ...
        "semantic_review_handoff": {
            "reviewer": "repository-local video-review",
