...
COMMON_ASSET_KEYS = frozenset(
    {"type", "description_en", "reuse_semantic_description_en", "visual"}
)
...
def contains_prohibited_style_shortcut(value: Any) -> bool:
    ...
    if isinstance(value, dict):
        return any(
            contains_prohibited_style_shortcut(item) for item in value.values()
        )
...
def _load_json(path: Path) -> dict[str, Any]:
    try:
        return load_json_object(
            path,
            label="required asset catalog",
            object_pairs_hook=_reject_duplicate_object_keys,
            error_type=StoryVideoError,
        )
...
def _reject_transport_fields(
    value: Any,
    location: str = "workspace/assets/assets.json",
) -> None:
    if isinstance(value, dict):
...
def load_asset_catalog(
    ...
    """Load the repository-owned ``workspace/assets/assets.json`` catalog.
    ...
    if catalog_path.is_symlink():
        raise StoryVideoError(
            "workspace/assets/assets.json must be a real file, "
            "not a symlink."
        )
    ...
    _require_exact_keys(
        payload, ROOT_KEYS, "workspace/assets/assets.json root"
    )
    ...
    if invalid:
        raise StoryVideoError(
            "workspace/assets/assets.json has invalid fixed fields: "
            + ", ".join(invalid)
        )
    if contains_prohibited_style_shortcut(payload):
        raise StoryVideoError(
            "workspace/assets/assets.json contains a prohibited "
    ...
    if not isinstance(raw_assets, dict) or not raw_assets:
        raise StoryVideoError(
            "workspace/assets/assets.json assets must be a non-empty "
            "object keyed by ID."
        )
    ...
    for asset_id, raw in raw_assets.items():
        ...
        if asset_type == "sound":
            ...
            continue
        normalized["reuse_semantic_description_en"] = require_utf8_text(
            value["reuse_semantic_description_en"],
            f"asset {asset_id} reuse_semantic_description_en",
        )
        ...
        if asset_type == "character" and "voice" in value:
