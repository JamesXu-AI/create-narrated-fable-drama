...
def identity_reference_audit(
    *, task_dir: Path, segment_id: str, plan: dict[str, Any]
    ...
    private_plan = read_json(
        task_dir / PLAN_DIR_RELATIVE / f"{segment_id}.json",
        label="private Seedance Segment plan",
    )
    binding_by_id = {
        item.get("binding_id"): item
        for item in private_plan.get("bindings", [])
...
def predecessor_observation_requirement(
    *, task_dir: Path, segment_id: str, plan: dict[str, Any]
    ...
    strong_reset = (
        shooting.get("seam_class") == "strong_coverage_reset"
    ...
    return {
        ...
        "private_plan_path": str(
            (task_dir / PLAN_DIR_RELATIVE / f"{segment_id}.json").resolve()
        ),
        ...
        "required_result": "seedance-video-review:NO_ISSUES",
    ...
    *,
    ...
    if actual != expected:
        raise SegmentRuntimeError(
            ...
            "resolved character/location bindings, receives seedance-video-review "
